...
class Dlsite(object):
    ...
    # 提取字符串中的 workno
    @staticmethod
    def parse_workno(string: str, *, folder_path: str | None = None):
        from file_ops import parse_rj_code, allow_bare_rj_digit_match
        allow_bare = False
        if folder_path:
            allow_bare = allow_bare_rj_digit_match(directory_roots=[folder_path])
        return parse_rj_code(string, allow_bare=allow_bare)
    ...
    @staticmethod
    def compile_product_api_url(rjcode: str):
